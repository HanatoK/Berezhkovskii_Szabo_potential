...
def plotfes(pmffilename, pngfilename, xtitle, ytitle):
    ...
    cf = plt.contourf(xi, yi, zi, levels=np.linspace(0, 8, 25), cmap='turbo')
    plt.xlabel(xtitle)
    plt.ylabel(ytitle)
    ...
    ax.xaxis.set_major_locator(plt.FixedLocator(np.linspace(-2, 1, 4)))
    ax.yaxis.set_major_locator(plt.FixedLocator(np.linspace(-1, 2, 4)))
    clb = plt.colorbar()
...
plotfes('potential.dat', 'potential.png', 'X', 'Y')
